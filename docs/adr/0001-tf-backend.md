...
Use the shared `cs-tfstate` Cloudflare R2 bucket as the Terraform backend (S3-compatible API). One state file per environment, keyed as `state-bucket/convergent-systems-co/event-atoms/<env>/terraform.tfstate`. CI runners authenticate via `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` repo secrets mapped to R2 API tokens.
